...
class Trainer:
    ...
    def Test(self, sentenceObj, dcsObj, dsbz2_name, _dump = False, _outFile = None):
        ...
        print("nodelist_correct",nodelist_correct)
        print("conflicts_Dict_correct",conflicts_Dict_correct)
        print("nodelist",nodelist)
        print("conflicts_Dict",conflicts_Dict)
        print("featVMat",featVMat)
        print("type",type(featVMat[0]))
        print("Node list Len",len(nodelist))
        print("Len featVMat",len(featVMat))
        print("Len featVMat[0]",len(featVMat[0]))
        print("Len featVMat[1]",len(featVMat[1]))
        #print("shape",featVMat[0].shape())
        exit(1)
...
#               format(word_match, lemma_match, len(dcsLemmas), len(nodelist)))
    ...
    def EBM(self, nodelist, conflicts_Dict, featVMat, _outFile = None):
        #print("ebm_train_clique")
        _dump = True
        if _dump:
            if _outFile is None:
                raise Exception('WTH r u thinking! pass me outFolder')
        ...
        print("mst_nodes : ",mst_nodes)
        return minScore, minMst
